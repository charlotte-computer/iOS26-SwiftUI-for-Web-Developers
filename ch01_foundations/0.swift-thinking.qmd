...
![](../public/images/build-fail.png){fig-align="center"}
...
```asm
; ARM64 machine code — generated by LLVM for an iPhone processor
; This is what actually ships inside the .ipa. Your source code is gone.

; ---- body getter: ContentView.body ----------------------------------------
; Swift calls this to ask "what UI should be on screen?"
_$s11ContentView4bodyQrvg:
    stp    x29, x30, [sp, #-32]!    ; save frame + link register
    mov    x29, sp

    ; Allocate a Swift.String for "Hello, World!"
    adrp   x0, l_.str@PAGE
    add    x0, x0, l_.str@PAGEOFF
    mov    x1, #13                  ; string byte length
    bl     _$sSS21_builtinStringLiteral17utf8CodeUnitCount7isASCIISSBp_BwBi1_tcfC

    ; Construct SwiftUI.Text(verbatim:) with that string
    bl     _$s7SwiftUI4TextV_E8verbatimACSS_tcfC

    ; Return the Text value as `some View` (existential box)
    ldp    x29, x30, [sp], #32
    ret

; ---- protocol witness table entry -----------------------------------------
; Tells the Swift runtime that ContentView conforms to the View protocol
_$s11ContentView7SwiftUI4ViewAAWP:
    .quad  _$s11ContentView4bodyQrvg   ; pointer to the body getter above
